...
func TestTrackRepo_GetByCompositionID(t *testing.T) {
    ...
    // Insert test tracks
    compositionID := "df004dcb-4207-43d6-a028-abb48991b61"
    _, err = db.Exec(`INSERT INTO tracks (composition_id, user_id, title, file_url, created_at, updated_at, deleted_at)
        VALUES ($1, 1, 'Track 1', 'https://example.com/1.mp3', now(), now(), 0),
               ($1, 1, 'Track 2', 'https://example.com/2.mp3', now(), now(), 0)
        ON CONFLICT (composition_id, title) DO NOTHING`, compositionID)
    assert.NoError(t, err)
